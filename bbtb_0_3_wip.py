  ...
  try:
      ...
      for comment in stream:
        if re.search("Frank", comment.body) and comment.id not in comments_replied_to:
          print '******* !BBT found in comment %s *******' % comment.id
          ...
          with open("comments_replied_to_test.txt", "w") as f:
            for comment_id in comments_replied_to:
              f.write(comment_id + "\n")
          print "Comment %s saved to comments_replied_to.txt" % comment.id
          
          print "Grabbing comment content"
          ...
          print "Content grabbed, saving to comments_saved.txt"
          #Save BBT, comment score, body, and author for future use
          comments_saved.append("[" + pst_time + "](" + permalink + ")" + " | " + score + "  \n" + body + "  \n" + username + "  ")
          with open("comments_saved_test.txt", "w") as f:
            for comment_content in comments_saved:
              f.write(comment_content + "\n\n")
          print "Content saved to comments_saved.txt"
          print "Done!"
